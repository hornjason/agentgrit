...
export async function runDaemonCycle(
  ...
  // 3d. Auto-generate rule-domains.json if missing or stale
  try {
    const { readGraph } = await import("../graph/builder");
    const { shouldAutoGenerate, autoGenerateDomains } = await import("../graph/auto-domains");
    const graph = readGraph();
    if (shouldAutoGenerate(graph)) {
      const adResult = await autoGenerateDomains(graph, { maxNodes: 50 });
      if (adResult.classified > 0) {
        console.log(`  auto-domains: ${adResult.classified} node(s) classified, wrote rule-domains.json`);
      }
    }
  } catch (err) {
    result.errors.push(`auto-domains: ${err instanceof Error ? err.message : String(err)}`);
  }
