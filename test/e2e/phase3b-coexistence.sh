...
if [ "$HOOK_COUNT" -ge 9 ]; then
  ...
  fail "Step 3: Only $HOOK_COUNT hooks, expected >= 9"
fi
...
if [ -f "$MANIFEST" ]; then
  ...
  if [ "$MANIFEST_COUNT" -eq 9 ]; then
    pass "Step 6: installed-hooks.json manifest has count=9"
  else
    fail "Step 6: Manifest count=$MANIFEST_COUNT, expected 9"
  fi
else
...
if [ "$AG_REFS" -ge 9 ] && [ "$PAI_REFS" -eq 0 ]; then
